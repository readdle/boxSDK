...
makeRequest = function (callb, path, data, headers, method, waitUntilDate) {
    ...
    if (waitUntilTime > now) {
        ...
        setTimeout(function () {
            ...
            makeRequest(callb, path, data, headers, method, waitUntilDate);
        }, waitUntilTime - now);
        return;
    }
...
uploadFile = function (callb, path, file, headers, waitUntilDate) {
    switch (true) {
        case (file.data == null):
            ...
            f.on('end', function () {
                uploadRequest(callb, path, file, headers, waitUntilDate);
            });
            break;
        default:
            uploadRequest(callb, path, file, headers, waitUntilDate);
...
function uploadRequest(callb, path, file, headers, waitUntilDate) {
    ...
    makeRequest(callb, path, multipart, headers, 'post', waitUntilDate);
}
